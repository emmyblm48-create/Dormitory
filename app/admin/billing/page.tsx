...
import { Loader2, Plus, Save, CheckCircle2, RotateCcw, Image as ImageIcon, X, DoorOpen } from "lucide-react";
...
export default function AdminBillingPage() {
  ...
  const groupedByRoom = useMemo(() => {
    const groups = new Map<string, ViewRoomBill[]>();
    for (const b of filtered) {
      const list = groups.get(b.room_number) ?? [];
      list.push(b);
      groups.set(b.room_number, list);
    }
    return Array.from(groups.entries())
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([room_number, roomBills]) => ({
        room_number,
        bills: roomBills.sort((a, b) => b.billing_month.localeCompare(a.billing_month)),
        outstanding: roomBills.filter((b) => b.status !== "paid").reduce((sum, b) => sum + b.grand_total, 0),
      }));
  }, [filtered]);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-sm font-bold text-slate-700">รายการบิลแยกตามห้อง</p>
        <select
          value={statusFilter}
              ...
              <div className="flex items-center justify-between px-1">
                ...
                {group.outstanding > 0 && (
                  <span className="text-xs font-semibold text-red-500">
                    ค้างชำระ {thb(group.outstanding)} บาท
                  </span>
                )}
              </div>
