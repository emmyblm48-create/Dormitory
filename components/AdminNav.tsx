...
export function AdminNav() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <nav className="hidden md:flex md:order-first md:flex-col md:w-56 md:shrink-0 border-r border-slate-200 bg-white py-4 gap-1">
          ...
          return (
            <Link
              ...
              className={`mx-3 flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                active ? "bg-blue-50 text-[#0B57D0]" : "text-slate-500 hover:bg-slate-50"
              }`}
            >
              <Icon size={18} />
              {label}
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden sticky bottom-0 z-20 bg-white border-t border-slate-200 flex overflow-x-auto shadow-[0_-2px_8px_rgba(0,0,0,0.04)]">
        {items.map(({ href, label, icon: Icon }) => {
          const active = href === "/admin" ? pathname === href : pathname.startsWith(href);
          return (
            <Link
